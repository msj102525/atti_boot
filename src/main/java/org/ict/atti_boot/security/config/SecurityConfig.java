    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http, TokenLoginRepository tokenLoginRepository) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers("/users/signup", "/login", "/notice", "/reissue", "/auth/kakao/callback","/auth/kakao/signup/callback","/file/view/*","/file/download/*","/doctor/*","/doctor/educations","/doctor", "/feed/*", "/feed", "/board/*","/board/boardList", "/board/boardDetail/*","/board/boardUpdate/*",
                                "/oneword/*", "/onewordsubject/*").permitAll() // 해당 경로들은 인증 없이 접근 가능합니다.
                        .requestMatchers(HttpMethod.DELETE, "/board/boardDetail/*").permitAll() // 모든 사용자에게 DELETE 요청 허용
